...
public class UdpSendMsgRunnable implements Runnable {
    ...
    public void run() {
        while (true) {
            try {
                ...
                MQMessage.MQEntity mqEntity = BrokerLocalCache.queueMap.get(queueName).take();
                // 等于 2 采用 udp 传输模式
                CopyOnWriteArraySet<InetSocketAddress> socketAddresses = BrokerLocalCache.packetQueueMap.get(queueName);
                if (null != socketAddresses) {
                    for (InetSocketAddress socket : socketAddresses) {
                        ...
                        if (mqEntity.getIsAck()) {
                            ...
                            boolean flag = ackSendMode(BrokerLocalCache.udpChannel, mqEntity, socket);
                            ...
                            BrokerLocalCache.udpChannel.writeAndFlush(MsgResolver.buildUdpDatagramPacket(mqEntity, socket));
                        }
                    }
    ...
    private synchronized void replyQueueInfo(MQMessage.MQEntity mqEntity, String queueName) throws InterruptedException {
        ...
        while (!BrokerLocalCache.queueMap.get(queueName).isEmpty()) {
            BrokerLocalCache.queueMap.get(queueName).offer(queue.poll());
        }
    }
}
